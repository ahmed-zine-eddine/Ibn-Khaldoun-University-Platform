...
import { BookOpen, User, Calendar, Filter } from 'lucide-react';
...
export default function StudentMyModules() {
        ...
        const filter =
          yearFilter === 'all'
            ...
            : yearFilter
              ...
              : {};
        const res = await academicAPI.myStudentEnseignements(filter);
  ...
  // Group enseignements by module so a student sees one card per module with
  // every (teacher, type) entry under it.
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Modules of your promo, with assigned teachers. Read-only.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-dashed border-slate-300 bg-white px-4 py-10 text-center text-sm text-slate-500">
          No modules found for your promo. If you were just enrolled, ask the admin to assign your modules.
        </div>
      ) : (
            ...
            <article key={`mod-${module?.id || rows[0].id}`} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <header className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <h2 className="truncate text-base font-semibold text-slate-900">{moduleTitle(module)}</h2>
                  {module?.semestre ? (
                ...
                {rows.map((row) => (
                  <li key={`row-${row.id}`} className="flex items-center justify-between gap-3 py-3">
                    <div className="flex min-w-0 items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-slate-800">{teacherName(row.enseignant)}</p>
